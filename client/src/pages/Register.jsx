...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Redirecting directly to Login after 1.5 seconds
            setTimeout(() => {
                navigate('/login');
            }, 1500);
        } catch (err) {
            ...
            <div className="relative z-10 w-full max-w-lg p-[1px] bg-gradient-to-br from-white/20 to-transparent rounded-3xl backdrop-blur-xl shadow-2xl mx-4">
                <div className="bg-[#1e293b]/95 p-8 md:p-10 rounded-3xl border border-white/10">
                    <div className="text-center mb-8">
                        <img src={logo} alt="TalkNest" className="h-14 mx-auto mb-4 hover:scale-110 transition-transform duration-300" />
                        <h2 className="text-3xl font-extrabold text-white tracking-tight">Create Account</h2>
                        <p className="text-slate-400 mt-2">Join the TalkNest community today</p>
                    </div>

                    <form onSubmit={handleSubmit} className="space-y-4">
                        {/* Name Field */}
                        <div className="relative group">
                            <HiOutlineUser className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 text-xl group-focus-within:text-teal-400 transition-colors" />
                            <input 
                                name="name" 
                                type="text" 
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="relative group">
                                <HiOutlinePhone className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 text-xl group-focus-within:text-teal-400 transition-colors" />
                                <input 
                                    name="phone" 
                                    type="tel" 
                                    placeholder="Phone" 
                        ...
                        <button 
                            disabled={loading} 
                            className="w-full py-4 bg-gradient-to-r from-teal-500 to-purple-600 text-white font-bold rounded-2xl shadow-[0_0_20px_rgba(20,184,166,0.3)] hover:shadow-[0_0_25px_rgba(168,85,247,0.4)] hover:opacity-95 active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed mt-4"
                        >
                    ...
                    <p className="mt-8 text-center text-slate-400 text-sm">
                        Already have an account? <Link to="/login" className="text-teal-400 font-bold hover:text-teal-300 transition-colors ml-1">Login</Link>
                    </p>
                </div>
            </div>
        </div>
